...
- After providing the region-specific answer, generate exactly two clear and concise follow-up questions, each explicitly asking if the user would like information for another specific region (e.g., "Would you like to see leave benefits for the Asia region?" and "Would you like to see leave benefits for the America region?").
...
Provide a complete and informative response that fully addresses the question with personalized context and proper folder categorization. At the end of your response, clearly list exactly two follow-up questions explicitly asking if the user would like information for another specific region."""
...
For questions about these documents, contact your HR department."""
    ...
    def generate_follow_up_actions(self, user_question: str, assistant_response: str, email: str, user_profile: Dict = None) -> List[str]:
        """Generate follow-up actions, coordinated for region-specific context"""
        actions = []
        region = user_profile.get('region', 'Unknown') if user_profile else 'Unknown'

        # If the response is region-specific, suggest follow-up for other regions
        if 'region' in assistant_response.lower() or region != 'Unknown':
            actions.append(f"Would you like to see benefits, policies, or expenses for a different region? If so, please specify the region.")
        # ...existing logic for other follow-ups...
        return actions
    ...
    def create_response_with_actions(self, response_text: str, user_question: str, email: str, user_profile: Dict = None) -> Dict[str, Any]:
        ...
        # Extract follow-up questions from the response using regex
        follow_up_questions = re.findall(r'(Would you like.*?\?)', response_text, re.IGNORECASE)
